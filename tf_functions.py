...
def semisupervised_train_loop(model, optimizer, train_dataset, avg_loss, mIoU, iters):
    ...
    for X, U in tqdm(train_dataset, total=iters):
        ...
        U_final = predict_labels(K, T, boxes, flip_mask, model, unlabeled_images)
        ...
        with tf.GradientTape() as tape:
            loss = combined_loss(i_mix, images, l, l_labeled, l_unlabeled, model)
            # loss = loss_s
        gradients = tape.gradient(loss, model.trainable_variables)
        optimizer.apply_gradients(grads_and_vars=zip(gradients, model.trainable_variables))
        ...
        valid_lbls, valid_preds = valid_mask_preds(labels, preds)
        mIoU.update_state(valid_lbls, valid_preds)
        for k in range(FLAGS.batch_size + FLAGS.unlabeled_batch_size * K):
...
@tf.function
def predict_labels(K, T, boxes, flip_mask, model, unlabeled_images):
...
# @tf.function
def combined_loss(i_mix, images, l, l_labeled, l_unlabeled, model):
    logits = model(i_mix)
